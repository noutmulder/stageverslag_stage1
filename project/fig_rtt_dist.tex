...
\draw[densely dashed] (7.333,0) -- (7.333,4.20); \node[font=\scriptsize,rotate=90,anchor=south] at (7.173,1.47) {limit 150\,ms};
% --- x ticks ---
...
\node[font=\small] at (4.00,-0.55) {Bluetooth round-trip time (ms)};
\node[font=\small,rotate=90] at (-0.95,2.10) {Round-trips (count)};
\end{tikzpicture}%
}
\caption{Distribution of Bluetooth round-trip times over campaign 2 (n=5872, 10\,ms bins). The pre-registered DV1(b) thresholds are P95 $<$ 150\,ms and P99 $<$ 250\,ms; the measured P95 (112\,ms) and P99 (134\,ms) both clear them with margin, and the full distribution lies below the 150\,ms limit except for a thin tail (max 167\,ms).}
\label{fig:rtt-hist}
\end{figure}
