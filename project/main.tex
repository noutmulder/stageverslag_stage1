...
\begin{itemize}[leftmargin=*]
  \item \textbf{Bend-transition error decomposition.} Instrument the bend
    ...
    arc-tracing scheme can be attributed individually. Equation~\ref{eq:arctracing-error}
    rules out the arc-tracing scheme as a dominant source at the current
    $5^\circ$ segment resolution, so a higher-order numerical integration
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textit{Architectural decomposition.} The five-layer pipeline isolates
    ...
    message-level corruption, connection drop, sensor noise, frame drop) maps
    to a single layer, which is what makes layer-local containment possible
    in principle. Demonstration of containment under adversarial conditions
    is a separate task that this report does not undertake.
...
without an accompanying measurement of the sensor noise spectrum. A short
pre-study with stationary encoder logging would have yielded a principled
starting point for tuning and is reflected in the recommendation for a
