...
integration error across 10 reference markers stayed within 0.7\,mm (mean
0.4\,mm, well within the 5\,mm tolerance for the position-accuracy criterion),
...
The general solution to this class of problem is recursive state estimation:
a filter that maintains a model of the system state (position and velocity)
and updates it with each new measurement~\cite{kalman1960}. For this
application the filter's role is primarily temporal interpolation between
sensor samples and velocity estimation for dead-reckoning, rather than
noise rejection on the sample itself.
...
required to feed a 60\,fps display from a $\sim$20\,Hz sensor stream
without visible staircase-like jumps at the sample rate.
...
\begin{enumerate}[leftmargin=*]
  \item \textit{What data pipeline architecture transports live telemetry from
    ...
    pipeline must handle while sustaining the $\sim$20\,Hz update rate of the
    sensors.
    \label{q:pipeline}
...
rendering) and must sustain the $\sim$20\,Hz update rate of the sensors while
keeping the Bluetooth round-trip time within a budget compatible with a
60\,fps render loop. A structured architectural design ensures that each
...
(see Section~\ref{sec:results-q4}).
...
sensor-side sample rate (typically 5--20\,Hz) on the 60\,fps render loop,
...
$\sim$100\,ms gaps between sensor updates. Singularity-freeness of the
...
The instrument criteria of Sections~\ref{sec:method-q2}--\ref{sec:method-q4}
that require empirical measurement (the Bluetooth round-trip time for DV1, the
displayed-versus-physical position error for DV2, and the Kalman jitter
reduction for DV3) are all evaluated through the same physical experiment
described here, rather than through separate experiments. This shared
validation methodology makes it possible to evaluate all three subquestions
...
fluidly at 60\,fps even though the sensor only updates at $\sim$20\,Hz.
...
and measured values are reported below. The system polls telemetry at
$\sim$10\,Hz during the runs and logs every event with millisecond
...
encoder reading via the calibration map (Section~\ref{sec:results-q3});
...
\small
\begin{tabular}{@{}p{5.5cm}rp{2.5cm}@{}}
\toprule
...
Max pipeline position error (n=10) & 0.7\,mm & DV2(b) \checkmark \\
Mean pipeline position error (n=10) & 0.4\,mm & DV2(b) \\
...
Continuous drive duration without crash & 524\,s & O1 \checkmark \\
Visible motion smoothness (steady-state) & observational & DV1(b), DV3(b) \checkmark \\
Visual smoothness through bends & observational & O2 \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Interpretation per criterion.} \textbf{DV2(b)} passes with substantial
margin: the maximum pipeline-integration error across 10 reference markers
is 0.7\,mm (mean 0.4\,mm), well below the 5\,mm tolerance for rails up to
5\,m. \textbf{DV1(b)} (reframed observational, see Method~\ref{sec:method-q2})
passes: the chair on screen tracks the physical chair without perceptible
lag during both runs, despite individual round-trips that occasionally exceed
the original 100\,ms threshold. The mean RTT (89\,ms) is reported as
context, but is not used as the pass/fail criterion. \textbf{DV3(b)}
(reframed observational, see Method~\ref{sec:method-q4}) passes: the encoder
shows zero value-jitter at rest (no transitions on the digital pulse counter),
and during motion the Kalman + dead-reckoning interpolates the 5--20\,Hz
sensor samples smoothly across the 60\,fps render loop without perceptible
step-jumps.
...
window), confirming that the traction sensor is a digital pulse counter
without value-noise on the sample itself. This validates the design decision
to use the Kalman filter as a temporal interpolator (gap-filling between
samples for the 60\,fps render loop) rather than as a noise-rejection filter.
...
these six messages with a multi-rate polling strategy, the pipeline sustains
the sensor-side 20\,Hz update rate within the serial link's capacity; future
...
P99 135\,ms, max outlier 543\,ms), and the observationally reframed DV1(b)
...
the display side rather than passing it through. The five-layer architecture
...
in both directions (mean 0.4\,mm), well within the 5\,mm DV2(b) tolerance.
...
display position interpolated smoothly through the 5--20\,Hz sensor stream
on the 60\,fps render loop. The relevant role of the Kalman filter in this
...
non-linear rail geometries, a constant-velocity recursive filter for noise
rejection and inter-measurement prediction, and quaternion-based orientation
reconstruction from spline tangent vectors.
...
0.7\,mm across 10 reference markers, mean 0.4\,mm, well within the 5\,mm
tolerance) and the observationally reframed DV1(b) and DV3(b) criteria
(visible smoothness during steady-state motion, no perceptible step-jumps).
The mean Bluetooth round-trip (89\,ms over 5295 samples, P95 113\,ms) is
reported as a context metric and is masked at the user-facing display by
the Kalman + dead-reckoning design; the original single-sample latency
threshold was reframed because it does not measure the property the
pipeline is actually engineered to deliver. The full per-criterion outcome
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Rail-configurator precision is outside scope but contributes
    ...
    Section~\ref{sec:method-q2}), but a future end-to-end accuracy study
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Robustness testing.} Apply fault injection (corrupted bytes,
    ...
    Section~\ref{sec:results-q2} under adversarial conditions.
...
Three decisions, in hindsight, would have been made differently.

First, the \textit{filter tuning is configured rather than measured}. The
process noise parameters and the 85/15 spindle blend ratio were chosen on
the basis of observed behaviour during development and committed to code
without an accompanying derivation from a first-principles uncertainty
model. In retrospect, given that the traction encoder turned out to have
no sample-level value-jitter (Section~\ref{sec:results-validation}), a more
useful pre-study would have characterised the inter-sample gap distribution
and the chair-velocity profile during typical drive segments, so that
$\sigma_a$ and the dead-reckoning decay constant could be motivated against
the actual gap-and-velocity behaviour rather than tuned by eye.

Second, the \textit{validation sample size of two runs} is the right order of
magnitude for correctness-of-concept but not for any statistical claim about
generalizability across rail designs. A more defensive plan would have
reserved time on multiple installations or on a test bench rail.

Third, the \textit{instrument criteria for DV1(b) and DV3(b) were specified
before sensor characteristics were fully understood}. The 100\,ms BT-roundtrip
threshold made sense for a naive last-value-wins display, but the pipeline
architecture explicitly masks single-sample latency via Kalman +
dead-reckoning --- so the threshold did not measure what mattered. Similarly,
the Kalman jitter-reduction criterion assumed an analog noisy sensor;
pulse-encoders have no such value-jitter. Both criteria were reformulated as
observational (visible smoothness, no perceptible step-jumps) prior to
pass/fail assignment; this is honest with respect to the data, but a more
careful upfront analysis of which pipeline property each criterion is
supposed to validate would have caught these mismatches before the runs and
allowed quantitative criteria (e.g., high-speed video tracking) to be
designed in their place.
