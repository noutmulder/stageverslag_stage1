...
technical problem is the mapping of noisy, asynchronous sensor readings onto
a curved 3D rail geometry with segment-dependent encoder-to-distance ratios.
A ``virtual ghost'' prototype was developed iteratively and a single
calibration run on physical hardware was used to verify that the deductive
chain of derivations holds under real-world conditions. The results show
that a processing pipeline combining non-linear encoder mapping, recursive
state estimation, and smooth tangent-based orientation reconstruction
produces a stable, accurate live 3D representation of stairlift position
and fault state.
\end{minipage}
\end{center}
\vspace{0.3cm}
...
require physical measurement. The prototype is validated through a single
calibration run on one physical stairlift installation; rail configuration
and measured values are reported below. During the run the lift is driven
from bottom station to top station and back, while the system polls telemetry
at 250\,ms intervals.

\textbf{Measurement methodology.} Physical chair position is read with a
steel tape (graduation 1\,mm, reading uncertainty $\approx\pm$1\,mm) at
reference points marked on the rail before the run (bottom station, top
station, and selected intermediate points at bend entries and exits). At
each reference point the displayed position in the visualization is compared
to the physical position; the absolute difference is the reported position
error. Bluetooth round-trip time is measured per request/response pair by
the application layer and reported as a run-average. Kalman-filtered jitter
is measured with the chair stationary at a station boundary, with the filter
running on the live encoder feed, over a 10\,s window. The same 10\,s
window is used to record the raw-encoder baseline jitter for comparison.
...
\begin{table}[H]
\centering
\caption{Validation measurements from the single calibration run on the physical stairlift. Bracketed placeholders are filled in at the time of the run.}
\label{tab:validation}
\small
\begin{tabular}{@{}p{4.0cm}rp{3.0cm}@{}}
\toprule
\textbf{Metric} & \textbf{Value} & \textbf{Pass/fail} \\
\midrule
Rail length & [meet]\,mm & --- \\
Number of horizontal bends & [meet] & --- \\
Position error at end station & [meet]\,mm & DV2(b) \\
Max position error at reference points & [meet]\,mm & DV2(b) \\
...
The pass/fail outcome per DV1--DV3 criterion is to be assigned after the
measurements have been collected: DV2(b) passes if the position error stays
below 5\,mm on a rail $\leq$ 5\,m or 10\,mm on a longer rail; DV3(b) passes
if the post-Kalman jitter is at least ten times smaller than the raw
baseline; DV1(b) passes if the Bluetooth round-trip time stays below
100\,ms.

\textbf{Logged events.} Any anomalies that occur during the run (Bluetooth
disconnects, parser checksum errors, latency spikes, manual interventions)
are recorded here at the time of the run for completeness.
...
without degrading position update rates. The pass/fail outcome of DV1(b)
against the measured Bluetooth round-trip time is reported in
Table~\ref{tab:validation}. The five-layer architecture itself isolates
...
handled by the transport layer; robustness under adversarial fault injection
was not exercised and is reported as a limitation.
...
encoder pulses and physical distance at bends. Whether the implementation
meets the DV2(b) tolerance on the validation rail is reported in
Table~\ref{tab:validation}. Equation~\ref{eq:arctracing-error} bounds the
per-segment arc-tracing error at $\approx 4.2\,\mu$m per segment, so any
position error materially above this accumulated bound is attributable to
the finite precision of the firmware pulse-cost constants
(Equation~\ref{eq:pulsecosts}, four to ten significant figures) or
mechanical backlash at bend transitions; neither is instrumented separately
in the present validation. This is reported as a limitation rather than
resolved.
...
representation. Whether the measured jitter reduction meets the DV3(b)
tenfold threshold is reported in Table~\ref{tab:validation}, alongside
observational notes on chair-motion smoothness (the practical signature of
singularity-free quaternion composition). The tuning parameters
...
The pass/fail outcome of every DV1--DV3 criterion on the validation run is
collected in Table~\ref{tab:validation}. The deductive answer to the central
question therefore reduces to the question whether the architecture and
mathematical models described in
Sections~\ref{sec:results-q2}--\ref{sec:results-q4} hold under the
measurement methodology defined in Section~\ref{sec:results-validation}.
Generalisation beyond the single validated rail configuration requires
additional runs and is out of scope for this report.

\subsection{Limitations}
\begin{itemize}[leftmargin=*]
  \item \textbf{Sample size.} The validation reported in this article is a
    single calibration run on one rail configuration. Statistical
    generalizability to the full range of rail designs is not established;
    additional runs on rails of differing length, bend count, and bend type
    would be required.
...
Second, the \textit{validation sample size of one run} is the right order of
