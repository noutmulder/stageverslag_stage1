...
or status bit field; and (b) the measured Bluetooth round-trip time on
physical hardware is below 100\,ms across the validation runs of
Section~\ref{sec:method-validation}. The 100\,ms threshold corresponds to the
approximate upper bound beyond which motion becomes visibly laggy to a human
operator in continuous tracking tasks; it is treated here as a working
...
the cited sources, and (b) the Kalman-filtered jitter on a stationary
telemetry signal is at least an order of magnitude smaller than the raw
encoder jitter, measured across the validation runs of
Section~\ref{sec:method-validation}. Singularity-freeness of the quaternion
composition is a theoretical property of the
...
require physical measurement. The prototype was validated through calibration
runs on three physical stairlift installations with rail configurations of
3.21, 4.82, and 6.45\,m and four, six, and eight horizontal bends
respectively. During each run the lift is driven from bottom station to top
station and back, while the system polls telemetry at 250\,ms intervals.

\textbf{Measurement methodology.} Physical chair position is read from a
calibrated steel tape (1\,mm graduation, $\pm$1\,mm reading uncertainty) fixed
along the rail before the run. Reference points are physically marked at the
bottom station, top station, and every bend entry/exit using adhesive markers
positioned with the same tape. At each reference point, the displayed
position in the visualization is logged simultaneously with the lift passing
the marker; the absolute difference is the reported position error. Because
the steel-tape reading uncertainty ($\pm$1\,mm) is a fixed component of every
reported position error, any error below approximately 2\,mm should be
interpreted as ``within measurement noise'' rather than as a meaningful
deviation. Bluetooth round-trip time is measured per request/response pair by
the application layer; the reported values are run-averages over the bulk of
each calibration run, excluding the first and last 5\,s to suppress
connection-setup and shutdown artefacts. Kalman-filtered jitter is measured
with the chair stationary at a station boundary, with the filter running on
the live encoder feed, over a 10\,s window.
...
Additional observations (frame rate, angle correction) are logged in the
table for completeness but are not pass/fail criteria.

\begin{table*}[t]
\centering
\caption{Validation measurements across three calibration runs on physical stairlift installations. The right-hand column reports pass/fail against the criterion in the originating method section; rows marked \textit{observation} are reported for completeness without an associated criterion.}
\label{tab:validation}
\small
\begin{tabular}{@{}p{4.2cm}rrrp{5.2cm}@{}}
\toprule
\textbf{Metric} & \textbf{Run 1 (6 bends)} & \textbf{Run 2 (4 bends)} & \textbf{Run 3 (8 bends)} & \textbf{Pass/fail vs.\ criterion} \\
\midrule
Rail length & 4\,820\,mm & 3\,210\,mm & 6\,450\,mm & --- \\
Total encoder range & 16\,200 pulses & 10\,790 pulses & 21\,680 pulses & --- \\
Position error at end station & $<$\,3\,mm & $<$\,2\,mm & $<$\,5\,mm & DV2(b): pass. \\
Max position error at bend transition & 4.2\,mm & 2.8\,mm & 6.1\,mm & DV2(b): pass (5\,mm threshold on rails $\leq$ 5\,m; 10\,mm on longer rails). \\
Encoder jitter (raw, stationary) & $\pm$\,2 pulses & $\pm$\,3 pulses & $\pm$\,2 pulses & DV3(b): baseline. \\
Encoder jitter (after Kalman filter) & $<$\,0.1 pulse & $<$\,0.1 pulse & $<$\,0.1 pulse & DV3(b): pass ($>$\,20$\times$ reduction). \\
Bluetooth round-trip time & 35\,ms avg & 42\,ms avg & 38\,ms avg & DV1(b): pass (all $\leq$ 100\,ms). \\
Render frame rate (desktop) & 60\,fps stable & 60\,fps stable & 60\,fps stable & Observation. \\
Render frame rate (tablet, Android 12+) & 55--60\,fps & 58--60\,fps & 52--58\,fps & Observation (see confound below). \\
Angle correction error (horizontal bends) & $0.3^\circ$ avg & $0.5^\circ$ avg & $0.4^\circ$ avg & Observation (see limitations). \\
\bottomrule
\end{tabular}
\end{table*}

All three runs satisfy every formal DV1--DV3 criterion. The 6.1\,mm
bend-transition error on Run~3 falls within the relaxed 10\,mm tolerance for
rails longer than 5\,m, but exceeds the 5\,mm threshold that applies to
shorter rails; this rail-length-dependent degradation is the basis for the
encoder-mapping discussion in Section~\ref{sec:conclusion}. The
Kalman-filtered jitter reduction factor exceeded the required tenfold on
every run.

\textbf{Logged events.} The parser checksum reported zero corruption events
across all three runs. Run~1: no anomalies. Run~2: one manual compass
...
\textbf{Side-by-side comparisons against earlier prototype iterations.} With
the piecewise encoder mapping of Section~\ref{sec:results-q3} enabled, no
segment-boundary jumps were visible in the rendered chair position. With an
85/15 blend between the Kalman-filtered spindle angle and the geometry-derived
pitch, no seat-tilt oscillation was visible during the bend passes; a 100\%
Kalman spindle angle variant, logged for comparison during Run~1, showed a
visible oscillation of approximately $\pm 0.4^\circ$ at bend entry and exit.

\textbf{Tablet frame rate confound.} On the tablet build, the shadow-quality
setting is reduced automatically by the rendering layer under
load~\cite{marek2015mobile3d}; the reported tablet frame-rate numbers
therefore reflect the system's adaptive behaviour, not a fixed-settings
benchmark.
...
without degrading position update rates. The measured Bluetooth round-trip
time of 35--42\,ms across the three runs is well below the 100\,ms working
threshold, satisfying DV1(b). The five-layer architecture itself isolates
failure modes by design: byte-level errors are caught by the protocol
parser's checksum, while higher-level issues (connection loss, timeout) are
handled by the transport layer. The Run~2 Bluetooth disconnect was recovered
transparently without corrupting the state estimator, which is consistent
with this isolation, although a single uncorrupted disconnect is observational
evidence rather than a proof of robustness under adversarial conditions.
...
encoder pulses and physical distance at bends. The validation data
(Table~\ref{tab:validation}) shows position errors below 5\,mm on the two
shorter rails (Run~1, Run~2) and within the relaxed 10\,mm tolerance on the
longer Run~3 rail, satisfying DV2(b). The 6.1\,mm bend-transition error on
Run~3, while within tolerance, is larger than the per-segment arc-tracing
error bound of Equation~\ref{eq:arctracing-error} ($\approx 4.2\,\mu$m per
segment, $<1$\,mm accumulated) can explain. The more plausible dominant
sources are the finite precision of the firmware pulse-cost constants
(Equation~\ref{eq:pulsecosts}, three to six significant figures) and
mechanical backlash at bend transitions; neither was instrumented separately
during these calibration runs. This is reported as a limitation rather than
resolved.

\textbf{Mathematical models.}
The constant-velocity Kalman filter suppressed stationary encoder jitter from
$\pm$2--3 raw pulses to below 0.1 pulse-equivalent across all three runs, a
reduction factor of more than twenty and well above the tenfold threshold of
DV3(b). Dead-reckoning extrapolation maintained responsiveness between
measurement updates. The Catmull-Rom tangent and quaternion composition
produced visibly smooth chair motion through the bend transitions; no
chair-snapping was observed during any of the three runs, which is the
practical signature of singularity-free quaternion composition. The tuning
parameters (Table~\ref{tab:kalman}) and the 85/15 spindle-pitch blend were
determined empirically rather than from a first-principles noise model;
sensitivity to these choices is therefore not formally bounded and is
...
Across the three validation runs, the implemented pipeline and models meet
every formal DV1--DV3 criterion: Bluetooth round-trip remains below 100\,ms,
displayed position falls within 5\,mm on rails up to 5\,m (within 10\,mm on
longer rails), and Kalman filtering reduces stationary encoder jitter by more
than a factor of twenty. The deductive answer to the central question is
therefore that the architecture and mathematical models described in
Sections~\ref{sec:results-q2}--\ref{sec:results-q4} are sufficient to produce
a stable, accurate live 3D visualization on the validated rail configurations,
within the measurement methodology defined in
Section~\ref{sec:results-validation}.

\subsection{Limitations}
\begin{itemize}[leftmargin=*]
  \item \textbf{Sample size.} The validation was performed on three rail
    configurations. Statistical generalizability to the full range of rail
    designs has not been established.
  \item \textbf{Empirical filter tuning.} The Kalman tuning parameters and the
    85/15 spindle blend ratio were determined empirically. A formal
    sensitivity analysis is absent.
  \item \textbf{Bend-transition error not decomposed.} The 6.1\,mm bend error
    on Run~3 stays within the relaxed 10\,mm tolerance for rails longer than
    5\,m, but exceeds the 5\,mm threshold that applies to shorter rails. The
    arc-tracing approximation is ruled out as a dominant source by
    Equation~\ref{eq:arctracing-error}; pulse-cost-constant precision and
    mechanical backlash were not measured separately.
  \item \textbf{Angle-correction measurement is self-referential.} The
    reported angle-correction error (0.3--0.5\,$^\circ$) is computed from the
    same compass tracker that generates the correction itself, so it is an
    internal consistency check rather than an independent validation. An
    external reference (e.g., a digital protractor or laser angle meter) would
    be required to validate the absolute accuracy of the correction.
  \item \textbf{Tablet frame-rate confound.} The tablet frame-rate numbers
    reflect the rendering layer's automatic shadow-quality degradation under
    load, not a fixed-settings benchmark. A repeat test with adaptive quality
    disabled would isolate the pipeline's intrinsic render performance from
    the engine's adaptive behaviour.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textit{Architectural decomposition.} The five-layer pipeline isolates
    concerns by design: each failure mode maps to a single layer. The Run~2
    Bluetooth disconnect was recovered transparently and the two latency
    spikes were absorbed by the state estimator's dead-reckoning, without
    propagating into visible artefacts in the rendered position
    (Section~\ref{sec:results-validation}).
  \item \textit{Validation against measurable thresholds.} The pass/fail
    criteria were defined in the method sections before the calibration runs,
    not calibrated to the results. Where measured values approached a
    threshold (the 6.1\,mm bend-transition error on Run~3 against the 10\,mm
    longer-rail tolerance), the proximity is reported and discussed rather
    than smoothed over.
\end{itemize}
