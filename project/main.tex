...
problem is the live mapping of noisy encoder, spindle, and swivel sensor
...
stable, live 3D representation that enables faster interpretation of
...
For state estimation from noisy sensor data, the Kalman filter is the
...
3D applications~\cite{kuipers1999quaternions}.
...
and must satisfy low-latency constraints. A structured architectural design
...
\node[block, right=0.6cm of transport] (parser) {\textbf{Protocol Parser}\\{\scriptsize 6-state FSM,}\\[-2pt]{\scriptsize checksum}};
\node[block, right=0.6cm of parser] (poller) {\textbf{Telemetry Poller}\\{\scriptsize 3 coroutines,}\\[-2pt]{\scriptsize 20\,Hz polling}};
\node[block, right=0.6cm of poller] (estimator) {\textbf{State Estimator}\\{\scriptsize 3$\times$ Kalman}\\[-2pt]{\scriptsize Filter}};
\node[block, right=0.6cm of estimator] (renderer) {\textbf{3D Renderer}\\{\scriptsize Ghost Chair}\\[-2pt]{\scriptsize + Ghost Stairs}};
