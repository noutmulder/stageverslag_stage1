...
\subsection{Fault Relevance (Documentation Analysis)}
\label{sec:method-q1}
\textbf{Method:} Formal analysis of the firmware protocol specification.\\
\textbf{Justification:} The stairlift controller firmware defines over 140
message types. The goal is not to observe the system in operation, but to
systematically catalogue the protocol as a formal specification and derive
which subset carries spatially interpretable information. This is a
documentation and specification analysis, not an observation of live
behaviour.\\
\textbf{Approach:} The message enumeration in the controller firmware source is
...
\textbf{Instrument:} A classification is considered complete when: (a) every
message ID in the firmware enumeration has been assigned to a category,
(b) for each message with a position, angle, or status bit field the data
layout is documented at byte-level, and (c) the subset relevant to the
visualization pipeline is explicitly identified with rationale.

\subsection{Data Pipeline Architecture (Design)}
\label{sec:method-q2}
\textbf{Method:} Architectural design.\\
\textbf{Justification:} The telemetry pipeline spans multiple layers (wireless
transport, binary protocol parsing, polling, state estimation, rendering) and
must satisfy the end-to-end timing budget: the Bluetooth round-trip is
10--80\,ms, the visualization must render at 60\,fps (16.7\,ms per frame), and
the combined latency must remain below the threshold at which the motion
becomes visibly laggy to a human operator. A structured architectural design
ensures that each layer's contribution to latency and reliability can be
...
failure modes. A diagram visualizes the data flow between layers.\\
\textbf{Instrument:} The design is considered sufficient when: (a) each layer
has a single, well-defined responsibility, (b) the expected end-to-end
latency from sensor read to display update is below 100\,ms, (c) the poller
sustains at least 20\,Hz per telemetry axis under normal Bluetooth conditions,
and (d) byte-level corruption is detectable and recoverable without propagating
to higher layers.
...
\subsection{Rendering Techniques (Design + Comparative Evaluation)}
\label{sec:method-q5}
\textbf{Method:} Design combined with structured comparative evaluation against
defined criteria.\\
\textbf{Justification:} The visualization must convey multiple simultaneous
data streams (position, tilt, rotation, accessory states) without overwhelming
the operator. Different rendering techniques encode information differently,
and the choice affects how quickly a fault can be identified. A formal
comparison against fixed criteria makes the selection reproducible rather than
subjective.\\
\textbf{Approach:} Candidate visualization techniques are implemented: a ghost
chair model following the rail, semi-transparent ghost stairs for spatial
context, a dither-clip effect for depth perception, rail annotations for
segment identification, and a frosted-glass UI for information panels. Each
technique is evaluated against four criteria: the type of information it
encodes, whether it supports direct spatial comparison between expected and
actual state, whether it adds or reduces visual clutter, and its computational
cost on the target hardware (desktop and tablet).\\
\textbf{Instrument:} The evaluation is considered complete when: (a) every
technique is scored on all four criteria in a single comparison matrix,
(b) the scoring is based on observable properties (not subjective preference),
and (c) the combination of techniques selected for the final prototype can be
justified from the matrix.

\subsection{Validation (Experiment)}
\label{sec:method-q6}
\textbf{Method:} Experiment on physical stairlift installations with predefined
pass/fail thresholds.\\
\textbf{Justification:} The mathematical models and pipeline architecture must
be verified under real-world conditions where mechanical tolerances, Bluetooth
variability, encoder quantization, and sensor noise all interact. Only a
physical experiment can establish that the theoretically correct models remain
correct in practice.\\
...
\textbf{Instrument:} A run is considered successful when all of the following
hold: (a) positional error at every reference point is below 5\,mm on rails up
to 5\,m and below 10\,mm on longer rails; (b) angle correction error at
horizontal bends is below $1^\circ$ on average; (c) Kalman-filtered jitter is
at least ten times smaller than raw encoder jitter; (d) end-to-end latency
from sensor read to display update is below 100\,ms; (e) render frame rate is
above 55\,fps on desktop and above 50\,fps on tablet. Runs that fail on any
criterion are reported with the cause.
...
live spatial visualization. These are
...
Position is updated using a trapezoidal midpoint
approximation~\cite{strang2016calculus}:
\begin{equation}
\label{eq:arctracing}
...
Five visualization techniques were implemented in a 3D
engine~\cite{linietsky2014godot}. Each technique encodes a different aspect of
the stairlift state:
...
Each technique was evaluated against four criteria defined in
Section~\ref{sec:method-q5}: the information it encodes, whether it supports
direct spatial comparison between expected and actual state, its effect on
visual clutter, and its computational cost on the target hardware. The results
are summarized in Table~\ref{tab:rendering}.

\begin{table*}[t]
\centering
\caption{Comparative evaluation of the five rendering techniques against the criteria defined in Section~\ref{sec:method-q5}. GPU cost is measured as relative frame time contribution on an Android 12+ tablet.}
\label{tab:rendering}
\small
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Technique} & \textbf{Information encoded} & \textbf{Supports expected vs. actual} & \textbf{Clutter effect} & \textbf{GPU cost} \\
\midrule
Ghost chair model & Position, tilt, swivel, accessories & Yes (chair vs. rail) & Neutral & Low \\
Ghost stair renderer & Staircase context, step count & Indirect (spatial anchor) & Adds context, no clutter & Medium \\
Dither-clip effect & Distance-based visibility & No & Reduces clutter & Low \\
Rail annotations & Segment types, stations & Yes (labelled reference) & Adds information & Very low \\
Liquid glass UI & Panel content only & No & Neutral (panels only) & Low \\
\bottomrule
\end{tabular}
\end{table*}

Figure~\ref{fig:ghost-chair} shows the combined visualization during operation.

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{images/ghost_chair_screenshot.png}
\caption{The virtual ghost visualization: the 3D ghost chair follows the rail based on live telemetry. Semi-transparent ghost stairs provide spatial context. Rail annotations identify segment boundaries along the rail.}
\label{fig:ghost-chair}
\end{figure*}

Of the five techniques, two (ghost chair model and rail annotations) directly
support the comparison between expected and actual state, which is the primary
diagnostic requirement. The ghost stair renderer provides spatial anchoring
indirectly by making the staircase visible, so the operator can interpret the
chair's position relative to physical landmarks. The dither-clip effect and
liquid glass UI do not contribute to fault identification directly, but they
reduce competing visual load so the diagnostic techniques stand out. The final
prototype therefore combines all five, with the ghost chair and rail
annotations as primary diagnostic elements and the others as supporting
context.
...
\begin{table*}[t]
\centering
\caption{Validation measurements across three calibration runs on physical stairlift installations. Observations column records deviations from the expected behaviour and any incidents during the run.}
\label{tab:validation}
\small
\begin{tabular}{@{}p{4.2cm}rrrp{5.2cm}@{}}
\toprule
\textbf{Metric} & \textbf{Run 1 (6 bends)} & \textbf{Run 2 (4 bends)} & \textbf{Run 3 (8 bends)} & \textbf{Observations} \\
\midrule
Rail length & 4\,820\,mm & 3\,210\,mm & 6\,450\,mm & \\
Total encoder range & 16\,200 pulses & 10\,790 pulses & 21\,680 pulses & \\
Position error at end station & $<$\,3\,mm & $<$\,2\,mm & $<$\,5\,mm & Run 3 at threshold of pass criterion. \\
Max position error at bend transition & 4.2\,mm & 2.8\,mm & 6.1\,mm & Run 3 exceeds 5\,mm target; correlates with bend count. \\
Angle correction error (horizontal bends) & $0.3^\circ$ avg & $0.5^\circ$ avg & $0.4^\circ$ avg & One bend in Run 2 required manual re-trigger after first compass reset failed. \\
Encoder jitter (raw, stationary) & $\pm$\,2 pulses & $\pm$\,3 pulses & $\pm$\,2 pulses & Higher raw jitter on Run 2 traced to a worn traction belt. \\
Encoder jitter (after Kalman filter) & $<$\,0.1 pulse & $<$\,0.1 pulse & $<$\,0.1 pulse & \\
Display update latency (BT round trip) & 35\,ms avg & 42\,ms avg & 38\,ms avg & Run 2 had two spikes $>$150\,ms during RF interference from a nearby WiFi access point. \\
Render frame rate (desktop) & 60\,fps stable & 60\,fps stable & 60\,fps stable & \\
Render frame rate (tablet, Android 12+) & 55--60\,fps & 58--60\,fps & 52--58\,fps & Drop below 55\,fps on Run 3 when all 8 bends framed at once. \\
Other incidents & --- & BT disconnect & --- & Run 2: BT disconnect at $\sim$60\,s, auto-reconnected in 2\,s, no data corruption. \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{Analysis of results.} Two of the three runs passed every pass/fail
criterion defined in Section~\ref{sec:method-q6}. Run 3 failed the 5\,mm
bend-transition threshold (6.1\,mm measured) and the 55\,fps tablet threshold
(52\,fps measured). Both failures are attributed to the rail length: the
arc-tracing approximation accumulates a small systematic error per bend, and
the longer ghost stair mesh increases GPU load. On rails up to 4.8\,m, the
position error remained within specification; above that, the error grows
proportionally to the number of bends.

The piecewise encoder mapping eliminated the position jumps at segment
boundaries that were observed during early testing with a linear mapping. The
Kalman filter reduced stationary encoder jitter from $\pm$\,2--3 raw pulses to
below 0.1 pulse equivalent in the display position, confirming that the chosen
process noise parameters ($\sigma_a = 0.03$ for traction) provide sufficient
smoothing without introducing perceptible lag. The display latency of
...
Not all observations were within expectation. Run 2 exhibited a raw encoder
jitter of $\pm$\,3 pulses, which was traced (after the run) to a worn traction
belt on that particular installation, not a sensor or pipeline issue. Run 2
also experienced a transient Bluetooth disconnect, which the transport layer
handled by reconnecting within two seconds; no data corruption propagated to
the state estimator, confirming the layered design's isolation of failure
modes. On the same run, RF interference from a nearby WiFi access point caused
two latency spikes above 150\,ms, temporarily visible as a short lag in the
ghost chair motion before the dead-reckoning extrapolation caught up.

The spindle geometry blend (85\% Kalman, 15\% rail pitch) prevented the seat
tilt oscillation that was observed during early testing with 100\% Kalman
