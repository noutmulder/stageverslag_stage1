...
The 3D engine choice determines the renderer capabilities, shader system,
mobile export pipeline, and the boundary between research code and third-party
code. Because this choice fixes input parameters for the design of
Section~\ref{sec:results-q5} (available materials, shader language, frame
loop), it is part of the vooronderzoek~\cite{gielingartikel2223}. Four
candidate engines were compared against four criteria: (C1) configurability
and extensibility (custom shaders, autoloads, plugin API access to the
renderer), (C2) cross-platform export to desktop (Windows, Linux) and mobile
(Android 12+) from a single source tree, (C3) licensing compatible with
closed-source commercial deployment without per-seat or revenue-share fees,
and (C4) native support for glTF binary asset import, which is the format in
which DeVi Comfort's mechanical CAD pipeline already
exports~\cite{khronos2021gltf}. The candidates and their scoring against
these criteria are shown in Table~\ref{tab:engine-mca}.
...
Godot 4.6 is selected because it is the only engine that meets all four
criteria simultaneously. Its MIT licence removes downstream royalty and
seat-fee risk for a company-owned service application; its export pipeline
produces a single tree that compiles to Windows, Linux, and Android with the
same shader code (using the \texttt{gl\_compatibility} renderer on mobile and
Forward+ on desktop); autoloads and shader globals give the configurability
needed for the dither-clip, frosted-glass, and rail-annotation shaders in
Section~\ref{sec:results-q5}; and its glTF binary import handles the
multi-mesh chair model without a format conversion step. Unreal was excluded
primarily on licence (royalty over revenue thresholds) and mobile export
friction; Unity on licence change risk after its 2023 runtime-fee
announcement~\cite{schreier2023unity}; Three.js on lack of a native Android
build target (the fallback of a WebView wrapper adds a transport layer and
breaks the gyroscope/accelerometer access required for the compass tracker of
...
\textbf{Why this method:} The stairlift controller firmware defines over 140
...
which subset carries spatially interpretable information. This is therefore a
formal analysis of documentation, not a sample-based observation study: the
input is the complete firmware enumeration, not sampled message frames from
live traffic.\\
...
\textbf{Why this approach:} Classifying on three independent axes (content,
rate, spatial interpretability) separates orthogonal properties that matter
for pipeline design: content selects parsing logic, rate selects polling
strategy, spatial interpretability selects relevance to the visualization. A
single-axis classification (``relevant yes/no'') would conflate these and
leave the poller design unsupported.\\
\textbf{Instrument (when done):} A classification is considered complete when:
(a) every message ID in the firmware enumeration has been assigned to a
category, (b) for each message with a position, angle, or status bit field the
data layout is documented at byte-level, and (c) the subset relevant to the
...
reliability can be analyzed independently.\\
...
estimation, collapsing concerns that the timing budget already separates.\\
...
Bluetooth conditions, and (d) byte-level corruption is detectable and
recoverable without propagating to higher layers.
...
\label{sec:method-q5}
\textbf{Method:} Design combined with structured comparative evaluation
against defined criteria.\\
\textbf{Why this method:} The visualization must convey multiple simultaneous
...
\textbf{Why this approach:} The evaluation is structured as an additive
scoring matrix rather than a weighted multi-criteria decision analysis (MCA).
The techniques do not compete for a single slot in the prototype; they layer
on top of each other in the renderer. The question therefore is not ``which
wins'' but ``which earn their frame-time cost''. An ordinal score per
criterion plus a measured GPU cost captures that trade-off directly; a
weighted MCA would suggest the techniques are mutually exclusive, which would
misrepresent the design.\\
\textbf{Instrument (when done):} The evaluation is considered complete when:
(a) every technique is scored on all four criteria in a single comparison
matrix, (b) the scoring is based on observable properties (not subjective
preference), and (c) the combination of techniques selected for the final
prototype can be justified from the matrix.
...
The five techniques are additive rather than alternative: the final
visualization combines all of them simultaneously, layered in the renderer.
For that reason a weighted multi-criteria decision analysis (MCA), which would
rank competing options, is not the appropriate instrument here; instead, each
technique is scored independently on a fixed four-criterion matrix and is then
kept or dropped based on whether its diagnostic contribution justifies its
frame-time cost on the weakest target (an Android~12+ tablet). The four
criteria match those defined in Section~\ref{sec:method-q5}. Scores use a
three-level ordinal scale ($+$ meets, $=$ partially meets, $-$ does not meet)
for the first three criteria and a measured relative GPU cost for the fourth.
GPU cost is expressed as the percentage contribution to total frame time,
averaged over a 60-second tablet capture with and without the technique
enabled. The results are summarized in Table~\ref{tab:rendering}.

\begin{table*}[t]
\centering
\caption{Evaluation of the five rendering techniques against the criteria from Section~\ref{sec:method-q5}. C1: information encoded (meets if the technique carries a distinct data stream). C2: supports direct expected-vs-actual comparison. C3: effect on visual clutter. Scores: $+$ meets, $=$ partially meets, $-$ does not meet. C4 GPU cost is the measured contribution to total frame time on the tablet target, averaged over a 60\,s capture (lower is better).}
\label{tab:rendering}
\small
\begin{tabular}{@{}lccccl@{}}
\toprule
\textbf{Technique} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4 GPU} & \textbf{Decision} \\
 & \textbf{Info} & \textbf{Exp.\,vs.\,actual} & \textbf{Clutter} & \textbf{cost} & \\
\midrule
Ghost chair model    & $+$ & $+$ & $=$ & 4\,\% & Keep (primary) \\
Ghost stair renderer & $+$ & $=$ & $+$ & 7\,\% & Keep (context) \\
Dither-clip effect   & $=$ & $-$ & $+$ & 2\,\% & Keep (context) \\
Rail annotations     & $+$ & $+$ & $=$ & 1\,\% & Keep (primary) \\
Liquid glass UI      & $=$ & $-$ & $=$ & 3\,\% & Keep (UX frame) \\
\bottomrule
\end{tabular}
\end{table*}
...
Two techniques (ghost chair model and rail annotations) score $+$ on C2 and
are therefore labelled primary diagnostic elements: they carry the direct
comparison between expected and actual state. The ghost stair renderer scores
$=$ on C2: it does not show expected-vs-actual by itself, but it makes the
physical staircase visible so the chair position can be interpreted against a
physical landmark. The dither-clip effect and liquid glass UI both score $-$
on C2 and are kept only because their C3 contribution (clutter reduction and
UX framing) protects the primary elements' legibility. The combined GPU cost
of 17\,\% leaves sufficient headroom against the 55\,fps tablet threshold
defined in Section~\ref{sec:method-q6} (validated in
Section~\ref{sec:results-q6}).
...
Second, the \textit{rendering evaluation is structurally weaker than the
numerical parts}. Where the mathematical models are validated against metric
thresholds, the rendering techniques are evaluated against a criteria matrix
whose scoring, while based on observable properties, is ultimately assessed by
the author. A small user study with even three to five colleagues would have
converted the inductive claim of ``more intuitive'' into a measurable diagnosis
time. The time cost was underestimated during planning.

Third, the \textit{sample size of three calibration runs} is the right order of
